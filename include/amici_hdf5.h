...
EXTERNC void AMI_HDF5_writeReturnData(const ReturnData *rdata,
...
EXTERNC UserData *AMI_HDF5_readSimulationUserDataFromFileName(
    const char *fileName, const char *datasetPath, Model *model);

EXTERNC UserData *AMI_HDF5_readSimulationUserDataFromFileObject(
...
EXTERNC ExpData *AMI_HDF5_readSimulationExpData(const char *hdffile,
                                                UserData *udata,
 ...
 * @return
 */
EXTERNC int AMI_HDF5_attributeExists(hid_t fileId, const char *datasetPath,
                                     const char *attributeName);

// Helper functions to reading and writing HDF5 attributes:

EXTERNC herr_t AMI_HDF5_createAndWriteDouble2DAttribute(
    hid_t dataset, const char *attributeName, const double *buffer, hsize_t m,
    hsize_t n);

EXTERNC herr_t AMI_HDF5_createAndWriteDouble3DAttribute(
    hid_t dataset, const char *attributeName, const double *buffer, hsize_t m,
    hsize_t n, hsize_t o);
...
EXTERNC void AMI_HDF5_getIntArrayAttribute(hid_t file_id,
                                           const char *optionsObject,
                                           const char *attributeName,
                                           int **destination, hsize_t *length);

EXTERNC void AMI_HDF5_getDoubleArrayAttribute2D(hid_t file_id,
                                                const char *optionsObject,
                                                const char *attributeName,
                                                double **destination,
                                                hsize_t *m, hsize_t *n);

EXTERNC int AMI_HDF5_getDoubleArrayAttribute3D(hid_t file_id,
                                               const char *optionsObject,
                                               const char *attributeName,
                                               double **destination, hsize_t *m,
                                               hsize_t *n, hsize_t *o);

EXTERNC void AMI_HDF5_getDoubleArrayAttribute4D(
...
EXTERNC void AMI_HDF5_setAttributeIntFromDouble(hid_t file_id,
                                                const char *obj_name,
