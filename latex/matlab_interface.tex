\label{matlab_interface_md_documentation_MATLAB_}%
\Hypertarget{matlab_interface_md_documentation_MATLAB_}%
 In the following we will give a detailed overview how to specify models in MATLAB and how to call the generated simulation files.\hypertarget{matlab_interface_autotoc_md45}{}\doxysubsection{Model Definition}\label{matlab_interface_autotoc_md45}
This guide will guide the user on how to specify models in MATLAB. For example implementations see the examples in the matlab/examples directory.\hypertarget{matlab_interface_autotoc_md46}{}\doxysubsubsection{Header}\label{matlab_interface_autotoc_md46}
...
\end{DoxyVerb}
\hypertarget{matlab_interface_autotoc_md47}{}\doxysubsubsection{Options}\label{matlab_interface_autotoc_md47}
...
\end{DoxyVerb}
\hypertarget{matlab_interface_autotoc_md49}{}\doxysubsubsection{Parameters}\label{matlab_interface_autotoc_md49}
...
\end{DoxyVerb}
\hypertarget{matlab_interface_autotoc_md50}{}\doxysubsubsection{Constants}\label{matlab_interface_autotoc_md50}
...
\end{DoxyVerb}
\hypertarget{matlab_interface_autotoc_md51}{}\doxysubsubsection{Differential Equation}\label{matlab_interface_autotoc_md51}
...
For DAEs also specify the mass matrix. \begin{DoxyVerb}model.sym.M = [1, 0, 0;...
                0, 1, 0;...
                0, 0, 0];
\end{DoxyVerb}
...
In the definition of the differential equation you can use certain symbolic functions. For a full list of available functions see {\ttfamily src/symbolic\+\_\+functions.\+cpp}.

Dirac functions can be used to cause a jump in the respective states at the specified time-\/point. This is typically used to model injections, or other external stimuli. Spline functions can be used to model time/state dependent response with unkown time/state dependence.\hypertarget{matlab_interface_autotoc_md52}{}\doxysubsubsection{Initial Conditions}\label{matlab_interface_autotoc_md52}
...
\end{DoxyVerb}
\hypertarget{matlab_interface_autotoc_md53}{}\doxysubsubsection{Observables}\label{matlab_interface_autotoc_md53}
...
In the definition of the observable you can use certain symbolic functions. For a full list of available functions see {\ttfamily src/symbolic\+\_\+functions.\+cpp}. Dirac functions in observables will have no effect.\hypertarget{matlab_interface_autotoc_md54}{}\doxysubsubsection{Events}\label{matlab_interface_autotoc_md54}
...
For more details about event support see \href{https://doi.org/10.1093/bioinformatics/btw764}{\texttt{ https\+://doi.\+org/10.\+1093/bioinformatics/btw764}}\hypertarget{matlab_interface_autotoc_md55}{}\doxysubsubsection{Standard Deviation}\label{matlab_interface_autotoc_md55}
...
Both {\ttfamily sigma\+\_\+y} and {\ttfamily sigma\+\_\+t} can either be a scalar or of the same dimension as the observables / events function. They can depend on time and parameters but must not depend on the states or observables. The values provided in {\ttfamily sigma\+\_\+y} and {\ttfamily sigma\+\_\+t} will only be used if the value in {\ttfamily D.\+Sigma\+\_\+Y} or {\ttfamily D.\+Sigma\+\_\+T} in the user-\/provided data struct is {\ttfamily NaN}. See simulation for details.\hypertarget{matlab_interface_autotoc_md56}{}\doxysubsubsection{Objective Function}\label{matlab_interface_autotoc_md56}
By default, AMICI assumes a normal noise model and uses the corresponding negative log-\/likelihood \begin{DoxyVerb}J = 1/2*sum(((y_i(t)-my_ti)/sigma_y_i)^2 + log(2*pi*sigma_y^2)
\end{DoxyVerb}
...
As reference see the default specification of {\ttfamily this.\+sym.\+Jy} in {\ttfamily \mbox{\hyperlink{classamimodel_a5aba9155145df4b3f70bfec887c25af9}{amimodel.\+make\+Syms}}}.\hypertarget{matlab_interface_autotoc_md57}{}\doxysubsubsection{SBML}\label{matlab_interface_autotoc_md57}
AMICI can also import SBML models using the command {\ttfamily SBML2\+AMICI}. This will generate a model specification as described above, which may be edited by the user to apply further changes.\hypertarget{matlab_interface_autotoc_md58}{}\doxysubsection{Model Compilation}\label{matlab_interface_autotoc_md58}
...
\item Then, from within the AMICI {\ttfamily matlab/} directory\+:
...
The integration status will be indicated by the sol.\+status flag. Negative values indicate failed integration. The log-\/likelihood will then be available as {\ttfamily sol.\+llh} and the derivative with respect to the parameters in {\ttfamily sol.\+sllh}. Notice that for adjoint sensitivities no state, observable and event sensitivities will be available. Yet this approach can be expected to be significantly faster for systems with a large number of parameters.\hypertarget{matlab_interface_autotoc_md65}{}\doxysubsubsection{Steady State Sensitivities}\label{matlab_interface_autotoc_md65}
