In the following we will give a detailed overview how to specify models in M\+A\+T\+L\+AB and how to call the generated simulation files.\hypertarget{matlab_interface_autotoc_md44}{}\doxysubsection{Model Definition}\label{matlab_interface_autotoc_md44}
This guide will guide the user on how to specify models in M\+A\+T\+L\+AB. For example implementations see the examples in the matlab/examples directory.\hypertarget{matlab_interface_autotoc_md45}{}\doxysubsubsection{Header}\label{matlab_interface_autotoc_md45}
...
\end{DoxyVerb}
\hypertarget{matlab_interface_autotoc_md46}{}\doxysubsubsection{Options}\label{matlab_interface_autotoc_md46}
...
\end{DoxyVerb}
\hypertarget{matlab_interface_autotoc_md48}{}\doxysubsubsection{Parameters}\label{matlab_interface_autotoc_md48}
...
\end{DoxyVerb}
\hypertarget{matlab_interface_autotoc_md49}{}\doxysubsubsection{Constants}\label{matlab_interface_autotoc_md49}
...
\end{DoxyVerb}
\hypertarget{matlab_interface_autotoc_md50}{}\doxysubsubsection{Differential Equation}\label{matlab_interface_autotoc_md50}
...
Dirac functions can be used to cause a jump in the respective states at the specified time-\/point. This is typically used to model injections, or other external stimuli. Spline functions can be used to model time/state dependent response with unkown time/state dependence.\hypertarget{matlab_interface_autotoc_md51}{}\doxysubsubsection{Initial Conditions}\label{matlab_interface_autotoc_md51}
...
\end{DoxyVerb}
\hypertarget{matlab_interface_autotoc_md52}{}\doxysubsubsection{Observables}\label{matlab_interface_autotoc_md52}
...
In the definition of the observable you can use certain symbolic functions. For a full list of available functions see {\ttfamily \mbox{\hyperlink{symbolic__functions_8cpp}{src/symbolic\+\_\+functions.\+cpp}}}. Dirac functions in observables will have no effect.\hypertarget{matlab_interface_autotoc_md53}{}\doxysubsubsection{Events}\label{matlab_interface_autotoc_md53}
...
For more details about event support see \href{https://doi.org/10.1093/bioinformatics/btw764}{\texttt{ https\+://doi.\+org/10.\+1093/bioinformatics/btw764}}\hypertarget{matlab_interface_autotoc_md54}{}\doxysubsubsection{Standard Deviation}\label{matlab_interface_autotoc_md54}
...
Both {\ttfamily sigma\+\_\+y} and {\ttfamily sigma\+\_\+t} can either be a scalar or of the same dimension as the observables / events function. They can depend on time and parameters but must not depend on the states or observables. The values provided in {\ttfamily sigma\+\_\+y} and {\ttfamily sigma\+\_\+t} will only be used if the value in {\ttfamily D.\+Sigma\+\_\+Y} or {\ttfamily D.\+Sigma\+\_\+T} in the user-\/provided data struct is {\ttfamily NaN}. See simulation for details.\hypertarget{matlab_interface_autotoc_md55}{}\doxysubsubsection{Objective Function}\label{matlab_interface_autotoc_md55}
...
As reference see the default specification of {\ttfamily this.\+sym.\+Jy} in {\ttfamily \mbox{\hyperlink{classamimodel_a5aba9155145df4b3f70bfec887c25af9}{amimodel.\+make\+Syms}}}.\hypertarget{matlab_interface_autotoc_md56}{}\doxysubsubsection{S\+B\+ML}\label{matlab_interface_autotoc_md56}
A\+M\+I\+CI can also import S\+B\+ML models using the command {\ttfamily S\+B\+M\+L2\+A\+M\+I\+CI}. This will generate a model specification as described above, which may be edited by the user to apply further changes.\hypertarget{matlab_interface_autotoc_md57}{}\doxysubsection{Model Compilation}\label{matlab_interface_autotoc_md57}
...
\end{DoxyItemize}\hypertarget{matlab_interface_autotoc_md60}{}\doxysubsection{Model Simulation}\label{matlab_interface_autotoc_md60}
...
The integration status will be indicated by the sol.\+status flag. Negative values indicate failed integration. The log-\/likelihood will then be available as {\ttfamily sol.\+llh} and the derivative with respect to the parameters in {\ttfamily sol.\+sllh}. Notice that for adjoint sensitivities no state, observable and event sensitivities will be available. Yet this approach can be expected to be significantly faster for systems with a large number of parameters.\hypertarget{matlab_interface_autotoc_md64}{}\doxysubsubsection{Steady State Sensitivities}\label{matlab_interface_autotoc_md64}
