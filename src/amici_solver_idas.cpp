...
int IDASolver::AMISensEEtolerances() {
    return IDASensEEtolerances(ami_mem);
}
...
int IDASolver::AMISetStabLimDet(int stldet) {
    return(0);
}

int IDASolver::AMISetStabLimDetB(int which, int stldet) {
    return(0);
}
...
int IDASolver::AMIReInitB(int which, realtype tB0, N_Vector yyB0, N_Vector ypB0) {
...
int IDASolver::AMIQuadSStolerancesB(int which, realtype reltolQB, realtype abstolQB) {
    return IDAQuadSStolerancesB(ami_mem, which, reltolQB, abstolQB);
}

int IDASolver::AMISolve(realtype tout, N_Vector yret, N_Vector ypret, realtype *tret, int itask) {
    return IDASolve(ami_mem, tout, tret, yret, ypret, itask);
}

int IDASolver::AMISolveF(realtype tout, N_Vector yret, N_Vector ypret, realtype *tret, int itask, int *ncheckPtr) {
...
int IDASolver::AMIDiag() {
    return(-99);
}

int IDASolver::AMIDiagB(int which) {
    return(-99);
}
...
int IDASolver::AMIGetNumNonlinSolvConvFails(void *ami_mem, long *numnonlinsolvconvfails) {
...
int IDASolver::AMICalcICB(int which, realtype tout1, N_Vector xB, N_Vector dxB) {
...
{
    return IDARootInit(ami_mem, 0, NULL);
}

int IDASolver::resultFunction(realtype tt, N_Vector yy, N_Vector yp, N_Vector rr, void *user_data) {
    TempData *tdata = (TempData *) user_data;

    return tdata->model->fxdot(tt, yy, yp, rr, user_data);
}

int IDASolver::resultFunctionB(realtype tt, N_Vector yy, N_Vector yp, N_Vector yyB, N_Vector ypB, N_Vector rrB, void *user_dataB) {
    TempData *tdata = (TempData *) user_dataB;

    return tdata->model->fxBdot(tt, yy, yp, yyB, ypB, rrB, user_dataB);
}

int IDASolver::rootFunction(realtype t, N_Vector y, N_Vector yp, realtype *gout, void *user_data)
{
    TempData *tdata = (TempData *) user_data;
    return tdata->model->froot(t, y, yp, gout, user_data);
}

int IDASolver::J(long N, realtype t, realtype c_j, N_Vector y, N_Vector yp, N_Vector r, DlsMat Jac, void *user_data, N_Vector tmp1, N_Vector tmp2, N_Vector tmp3)
{
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fJ(N, t, c_j, y, r, yp, Jac, user_data, tmp1, tmp2, tmp3);
}

int IDASolver::fqBdot(realtype t, N_Vector x, N_Vector dx, N_Vector xB, N_Vector dxB, N_Vector qBdot, void *user_data)
{
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fqBdot(t, x, xB, qBdot, user_data);
}

int IDASolver::fsxdot(int Ns, realtype t,
                      N_Vector x, N_Vector xdot, N_Vector dx,
                      N_Vector *sx, N_Vector *sxdot, N_Vector *sdx, void *user_data,
                      N_Vector tmp1, N_Vector tmp2, N_Vector tmp3)
{
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fsxdot(Ns, t, x, xdot, 0, *sx, *sxdot, user_data, tmp1, tmp2);
}

int IDASolver::fJSparse(realtype t, realtype cj, N_Vector x, N_Vector dx, N_Vector xdot, SlsMat J, void *user_data, N_Vector tmp1, N_Vector tmp2, N_Vector tmp3)
{
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fJSparse(t, x, xdot, J, user_data, tmp1, tmp2, tmp3);
}

int IDASolver::fJBand(long int N, long int mupper, long int mlower, realtype t, realtype cj, N_Vector x, N_Vector dx, N_Vector xdot, DlsMat J, void *user_data, N_Vector tmp1, N_Vector tmp2, N_Vector tmp3)
{
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fJBand(N, mupper, mlower, t, x, xdot, J, user_data, tmp1, tmp2, tmp3);
}

int IDASolver::fJv(realtype t, N_Vector x, N_Vector dx, N_Vector xdot, N_Vector v, N_Vector Jv, realtype cj, void *user_data, N_Vector tmp1, N_Vector tmp2)
{
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fJv(v, Jv, t, x, xdot, user_data, tmp1);
}

int IDASolver::fJB(long int NeqBdot, realtype t, realtype cj, N_Vector x, N_Vector dx, N_Vector xB, N_Vector dxB, N_Vector xBdot, DlsMat JB, void *user_data, N_Vector tmp1B, N_Vector tmp2B, N_Vector tmp3B)
{
    TempData *tdata = (TempData *) user_data;
...
int IDASolver::fJSparseB(realtype t, realtype cj, N_Vector x, N_Vector dx, N_Vector xB, N_Vector dxB, N_Vector xBdot, SlsMat JB, void *user_data, N_Vector tmp1B, N_Vector tmp2B, N_Vector tmp3B)
{
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fJSparseB(t, x, xB, xBdot, JB, user_data, tmp1B, tmp2B, tmp3B);
}

int IDASolver::fJBandB(long int NeqBdot, long int mupper, long int mlower, realtype t, realtype cj, N_Vector x, N_Vector dx, N_Vector xB, N_Vector dxB, N_Vector xBdot, DlsMat JB, void *user_data, N_Vector tmp1B, N_Vector tmp2B, N_Vector tmp3B)
{
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fJBandB(NeqBdot, mupper, mlower, t, x, xB, xBdot, JB, user_data, tmp1B, tmp2B, tmp3B);
}

int IDASolver::fJvB(realtype t, N_Vector x, N_Vector dx, N_Vector xB, N_Vector dxB, N_Vector xBdot, N_Vector vB, N_Vector JvB, realtype cj, void *user_data, N_Vector tmpB1, N_Vector tmpB2)
{
    TempData *tdata = (TempData *) user_data;
