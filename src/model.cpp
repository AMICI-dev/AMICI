...
Model::Model(const int nx_rdata, const int nxtrue_rdata, const int nx_solver,
    ...
    : nx_rdata(nx_rdata), nxtrue_rdata(nxtrue_rdata), nx_solver(nx_solver),
      ...
      ndxdotdw(ndxdotdw), ndxdotdp_explicit(ndxdotdp_explicit),
      ndxdotdp_implicit(ndxdotdp_implicit), ndJydy(std::move(ndJydy)),
      nnz(nnz), nJ(nJ), ubw(ubw), lbw(lbw), pythonGenerated(pythonGenerated), o2mode(o2mode),
      idlist(std::move(idlist)), J(nx_solver, nx_solver, nnz, CSC_MAT),
      dxdotdw(nx_solver, nw, ndxdotdw, CSC_MAT), dwdp(nw, p.size(), ndwdp, CSC_MAT),
