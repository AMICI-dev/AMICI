    ...
    for (int i_node = 0; i_node < n_nodes() - 2; i_node++) {
        ...
        for (int ip = 0; ip < nplist; ip++)
            getCoeffsSensiLowlevel(ip, i_node, offset, len, len_m, len_p, 
                                   dnodesdp, dslopesdp, coefficients_sensi,
                                   coefficients_extrapolate_sensi);
    }
...
void HermiteSpline::getCoeffsSensiLowlevel(int ip, int i_node, int offset, 
    ...
    if (get_node_derivative_by_FD()) {
        if (i_node == 0) {
            ...
            smk1 = (dnodesdp[nodes_offset + i_node + 2] - spk) / len_p;
            
        } else if (i_node == n_nodes() - 1) {
            /* Are we at the last node? What's the boundary condition? */
            smk = (spk1 - dnodesdp[nodes_offset + i_node - 1]) / len_m;
