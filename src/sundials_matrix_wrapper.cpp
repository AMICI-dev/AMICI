...
SUNMatrixWrapper::SUNMatrixWrapper(const SUNMatrixWrapper &A,
                ...
                case CSR_MAT:
                    for (sunindextype i = 0; i < rows(); ++i) {
                        for (sunindextype k = indexptrs()[i]; k < indexptrs()[i+1]; ++k){
                            c.at(i) += data()[k] * b.at(indexvals()[k]);
                        }
                    }
                    break;
            }
