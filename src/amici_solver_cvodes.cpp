...
int CVodeSolver::AMISetId(Model *model) {
    return(0);
}

int CVodeSolver::AMISetSuppressAlg(bool flag) {
    return(0);
}
...
int CVodeSolver::AMIReInitB(int which, realtype tB0, N_Vector yyB0, N_Vector ypB0) {
    return CVodeReInitB(ami_mem, which, tB0, yyB0);
}

int CVodeSolver::AMISStolerancesB(int which, realtype relTolB, realtype absTolB) {
...
int CVodeSolver::AMIQuadSStolerancesB(int which, realtype reltolQB, realtype abstolQB) {
    return CVodeQuadSStolerancesB(ami_mem, which, reltolQB, abstolQB);
}

int CVodeSolver::AMISolve(realtype tout, N_Vector yret, N_Vector ypret, realtype *tret, int itask) {
    return CVode(ami_mem, tout, yret, tret, itask);
}

int CVodeSolver::AMISolveF(realtype tout, N_Vector yret, N_Vector ypret, realtype *tret, int itask, int *ncheckPtr) {
...
int CVodeSolver::AMIDense(int nx) {
...
int CVodeSolver::AMIDiag() {
    return CVDiag(ami_mem);
}

int CVodeSolver::AMIDiagB(int which) {
...
int CVodeSolver::AMIGetNumNonlinSolvConvFails(void *ami_mem, long *numnonlinsolvconvfails) {
...
int CVodeSolver::AMICalcIC(realtype tout1) {
    return(0);
}

int CVodeSolver::AMICalcICB(int which, realtype tout1, N_Vector xB, N_Vector dxB) {
    return(0);
}
...
int CVodeSolver::resultFunction(realtype t, N_Vector y, N_Vector ydot, void *user_data) {
    TempData *tdata = (TempData *) user_data;

    return tdata->model->fxdot(t, y, NULL, ydot, user_data);
}

int CVodeSolver::resultFunctionB(realtype t, N_Vector y, N_Vector yB, N_Vector yBdot, void *user_dataB) {
    TempData *tdata = (TempData *) user_dataB;

    return tdata->model->fxBdot(t, y, NULL, yB, NULL, yBdot, user_dataB);
}

int CVodeSolver::rootFunction(realtype t, N_Vector x, realtype *root, void *user_data) {
    TempData *tdata = (TempData *) user_data;
    return tdata->model->froot(t, x, NULL, root, user_data);
}

int CVodeSolver::J(long N, realtype t, N_Vector x, N_Vector xdot, DlsMat J, void *user_data, N_Vector tmp1, N_Vector tmp2, N_Vector tmp3) {
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fJ(N, t, 0.0, x, NULL, xdot, J, user_data, tmp1, tmp2, tmp3);
}

int CVodeSolver::fqBdot(realtype t, N_Vector x, N_Vector xB, N_Vector qBdot, void *user_data) {
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fqBdot(t, x, xB, qBdot, user_data);
}

int CVodeSolver::fsxdot(int Ns, realtype t, N_Vector x, N_Vector xdot, int ip, N_Vector sx, N_Vector sxdot, void *user_data, N_Vector tmp1, N_Vector tmp2) {
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fsxdot(Ns, t, x, xdot, ip, sx, sxdot, user_data, tmp1, tmp2);
}

int CVodeSolver::fJSparse(realtype t, N_Vector x, N_Vector xdot, SlsMat J, void *user_data, N_Vector tmp1, N_Vector tmp2, N_Vector tmp3) {
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fJSparse(t, x, xdot, J, user_data, tmp1, tmp2, tmp3);
}

int CVodeSolver::fJBand(long N, long mupper, long mlower, realtype t, N_Vector x, N_Vector xdot, DlsMat J, void *user_data, N_Vector tmp1, N_Vector tmp2, N_Vector tmp3) {
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fJBand(N, mupper, mlower, t, x, xdot, J, user_data, tmp1, tmp2, tmp3);
}

int CVodeSolver::fJv(N_Vector v, N_Vector Jv, realtype t, N_Vector x, N_Vector xdot, void *user_data, N_Vector tmp) {
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fJv(v, Jv, t, x, xdot, user_data, tmp);
}

int CVodeSolver::fJB(long int NeqBdot, realtype t, N_Vector x, N_Vector xB, N_Vector xBdot, DlsMat JB, void *user_data, N_Vector tmp1B, N_Vector tmp2B, N_Vector tmp3B) {
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fJB(NeqBdot, t, x, xB, xBdot, JB, user_data, tmp1B, tmp2B, tmp3B);
}

int CVodeSolver::fJSparseB(realtype t, N_Vector x, N_Vector xB, N_Vector xBdot, SlsMat JB, void *user_data, N_Vector tmp1B, N_Vector tmp2B, N_Vector tmp3B) {
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fJSparseB(t, x, xB, xBdot, JB, user_data, tmp1B, tmp2B, tmp3B);
}

int CVodeSolver::fJBandB(long int NeqBdot, long int mupper, long int mlower, realtype t, N_Vector x, N_Vector xB, N_Vector xBdot, DlsMat JB, void *user_data, N_Vector tmp1B, N_Vector tmp2B, N_Vector tmp3B) {
    TempData *tdata = (TempData *) user_data;
    return tdata->model->fJBandB(NeqBdot, mupper, mlower, t, x, xB, xBdot, JB, user_data, tmp1B, tmp2B, tmp3B);
}

int CVodeSolver::fJvB(N_Vector vB, N_Vector JvB, realtype t, N_Vector x, N_Vector xB, N_Vector xBdot, void *user_data, N_Vector tmpB) {
    TempData *tdata = (TempData *) user_data;
