...
% USAGE:
% ======
% [...] = simulate_model_dirac_adjoint_hessVecProd(tout,theta)
% [...] = simulate_model_dirac_adjoint_hessVecProd(tout,theta,kappa,data,options)
% [status,tout,x,y,sx,sy] = simulate_model_dirac_adjoint_hessVecProd(...)
%
% INPUTS:
% =======
...
function varargout = simulate_model_dirac_adjoint_hessVecProd(varargin)
...
if(options_ami.sensi<2)
    sol = ami_model_dirac_adjoint_hessVecProd(tout,theta(1:4),kappa(1:0),options_ami,plist,pbar,xscale,data);
else
    sol = ami_model_dirac_adjoint_hessVecProd_o2vec(tout,theta(1:4),kappa(1:4),options_ami,plist,pbar,xscale,data);
end
if(options_ami.sensi==1)
