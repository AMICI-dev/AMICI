...
% USAGE:
% ======
% [...] = simulate_model_adjoint_hessian(tout,theta)
% [...] = simulate_model_adjoint_hessian(tout,theta,kappa,data,options)
% [status,tout,x,y,sx,sy] = simulate_model_adjoint_hessian(...)
%
% INPUTS:
% =======
...
function varargout = simulate_model_adjoint_hessian(varargin)
...
if(options_ami.sensi<2)
    sol = ami_model_adjoint_hessian(tout,theta(1:3),kappa(1:0),options_ami,plist,pbar,xscale,data);
else
    sol = ami_model_adjoint_hessian_o2(tout,theta(1:3),kappa(1:0),options_ami,plist,pbar,xscale,data);
end
if(options_ami.sensi==1)
...
if(options_ami.sensi == 2)
    if(options_ami.sensi_meth==2)
        ...
        sol.s2llh = sol.s2llh.*(chainRuleFactor*transpose(chainRuleFactor)) + diag(sol.sllh*log(10));
        sol.sx = permute(reshape(transpose(sol.x(:,1+1:end)), 1, 3, data.nt), [3,1,2]);
        sol.sx = bsxfun(@times,sol.sx,permute(chainRuleFactor,[3,2,1]));
        sol.sy = permute(reshape(transpose(sol.y(:,1+1:end)), 1, 3, data.nt), [3,1,2]);
        sol.sy = bsxfun(@times,sol.sy,permute(chainRuleFactor,[3,2,1]));
        sol.sz = permute(reshape(transpose(sol.z(:,0+1:end)), 0, 3, data.ne), [3,1,2]);
        sol.sz = bsxfun(@times,sol.sz,permute(chainRuleFactor,[3,2,1]));
        sol.ssigmay = bsxfun(@times,sol.ssigmay,permute(chainRuleFactor,[3,2,1]));
        sol.ssigmaz = bsxfun(@times,sol.ssigmaz,permute(chainRuleFactor,[3,2,1]));
        sol.x = sol.x(:,1:1);
        sol.y = sol.y(:,1:1);
        sol.z = sol.z(:,1:0);
    else
        ...
        s2x = reshape(sol.sx(:,2:end,:),length(tout),1,length(theta(options_ami.sens_ind)),length(theta(options_ami.sens_ind)));
        s2y = reshape(sol.sy(:,2:end,:),length(tout),1,length(theta(options_ami.sens_ind)),length(theta(options_ami.sens_ind)));
        s2sigmay = reshape(sol.ssigmay(:,2:end,:),length(tout),1,length(theta(options_ami.sens_ind)),length(theta(options_ami.sens_ind)));
        s2z = zeros(size(sol.z,1),0,length(theta(options_ami.sens_ind)),length(theta(options_ami.sens_ind)));
        s2sigmaz = zeros(size(sol.z,1),0,length(theta(options_ami.sens_ind)),length(theta(options_ami.sens_ind)));
        s2rz = zeros(size(sol.z,1),0,length(theta(options_ami.sens_ind)),length(theta(options_ami.sens_ind)));
        for iz = 1:0
            ...
            s2sigmaz(:,iz,:,:) = reshape(sol.ssigmaz(:,((iz-1)*(length(theta(options_ami.sens_ind)+1))+2):((iz-1)*(length(theta(options_ami.sens_ind)+1))+length(theta(options_ami.sens_ind))+1),:),options_ami.nmaxevent,1,length(theta(options_ami.sens_ind)),length(theta(options_ami.sens_ind)));
            s2rz(:,iz,:,:) = reshape(sol.srz(:,((iz-1)*(length(theta(options_ami.sens_ind)+1))+2):((iz-1)*(length(theta(options_ami.sens_ind)+1))+length(theta(options_ami.sens_ind))+1),:),options_ami.nmaxevent,1,length(theta(options_ami.sens_ind)),length(theta(options_ami.sens_ind)));
        end
        sol.x = sol.x(:,1:1);
        sol.y = sol.y(:,1:1);
        sol.z = sol.z(:,1:0);
        ...
        sol.s2sigmay = bsxfun(@times,s2sigmay,permute(theta(options_ami.sens_ind)*transpose(theta(options_ami.sens_ind))*(log(10)^2),[4,3,2,1])) + bsxfun(@times,ssigmay,permute(diag(log(10)^2*theta(options_ami.sens_ind).*ones(length(theta(options_ami.sens_ind)),1)),[4,3,2,1]));
        ...
        sol.s2sigmaz = bsxfun(@times,s2sigmaz,permute(theta(options_ami.sens_ind)*transpose(theta(options_ami.sens_ind))*(log(10)^2),[4,3,2,1])) + bsxfun(@times,ssigmaz,permute(diag(log(10)^2*theta(options_ami.sens_ind).*ones(length(theta(options_ami.sens_ind)),1)),[4,3,2,1]));
