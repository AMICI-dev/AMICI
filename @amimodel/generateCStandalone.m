...
function generateMainC(this)
    ...
    fprintf(fid, '    nt = 20;\n');
    fprintf(fid, '    ts = linSpaceAlloc(0, 300, nt);');
    fprintf(fid, '\n');
    fprintf(fid, '    /* parameters, matlab: second argument */\n');
    fprintf(fid, '    //    np = (int) mxGetM(prhs[4]) * mxGetN(prhs[4]);\n');
    fprintf(fid, '    np = 5;\n');
    fprintf(fid, '    p = malloc(sizeof(realtype) * np); // theta\n');
    fprintf(fid, '    p[0] = 1;\n');
    fprintf(fid, '    p[1] = 0.5;\n');
    fprintf(fid, '    p[2] = 0.4;\n');
    fprintf(fid, '    p[3] = 2;\n');
    fprintf(fid, '    p[4] = 0.1;\n');
    fprintf(fid, '    for(int i = 0; i < np; ++i) p[i] = log10(p[i]);\n');
    fprintf(fid, '\n');
    ...
    fprintf(fid, '    const int numK = 4;\n');
    fprintf(fid, '    k = malloc(sizeof(realtype) * numK); // empty matrix if no fix parameters?!\n');
    fprintf(fid, '    k[0] = 0.1;\n');
    fprintf(fid, '    k[1] = 0.4;\n');
    fprintf(fid, '    k[2] = 0.7;\n');
    fprintf(fid, '    k[3] = 1;\n');
    ...
    fprintf(fid, '    }\n');
    fprintf(fid, '\n');
    fprintf(fid, '    int sizeTheta = 5;\n');
    fprintf(fid, '    int numTimepoints = 20;\n');
    fprintf(fid, '\n');
    fprintf(fid, '    // observation sizeTheta * numTimepoints\n');
    fprintf(fid, '    my = malloc(sizeof(realtype) * numTimepoints * nx);\n');
    fprintf(fid, '    fillArray(my, numTimepoints * nx, NAN);\n');
    fprintf(fid, '//    my[0] = 0.1;\n');
    fprintf(fid, '//    my[1] = 0.4;\n');
    fprintf(fid, '//    my[2] = 0.7;\n');
    fprintf(fid, '\n');
    fprintf(fid, '    // stdev of Y\n');
    fprintf(fid, '    ysigma =  malloc(sizeof(realtype) * numTimepoints * nx);\n');
    fprintf(fid, '    fillArray(ysigma, numTimepoints * nx, NAN);\n');
    fprintf(fid, '\n');
    ...
    fprintf(fid, '    for(int i = 0; i < nt; ++i)\n');
    ...
    fprintf(fid, '            printf("%%f\\t", rdata->am_xdata[j +  nt * i]);\n');
    fprintf(fid, '        printf("\\n");\n');
    fprintf(fid, '    }\n');
    fprintf(fid, '\n');
    fprintf(fid, '    printf("\\nydata\\n");\n');
    fprintf(fid, '    for(int i = 0; i < nx; ++i) {\n');
    fprintf(fid, '        for(int j = 0; j < nt; ++j)\n');
    fprintf(fid, '            printf("%%f\\t", rdata->am_ydata[j +  nt * i]);\n');
    fprintf(fid, '        printf("\\n");\n');
    fprintf(fid, '    }\n');
    fprintf(fid, '\n');
    fprintf(fid, '    printf("\\n\\nxdotdata: ");\n');
    ...
    fprintf(fid, '            printf("%%f\\t", rdata->am_Jdata[i + nx * j]);\n');
    fprintf(fid, '        printf("\\n");\n');
    fprintf(fid, '    }\n');
    fprintf(fid, '\n');
    fprintf(fid, '    printf("\\nnumsteps: \\t\\t");\n');
    fprintf(fid, '    printfArray(numstepsdata, nt, "%%d ");\n');
    fprintf(fid, '    printf("\\nnumrhsevalsdata: \\t");\n');
    fprintf(fid, '    printfArray(numrhsevalsdata, nt, "%%d ");\n');
    fprintf(fid, '    printf("\\norder: \\t\\t");\n');
    fprintf(fid, '    printfArray(orderdata, nt, "%%d ");\n');
    fprintf(fid, '\n');
    fprintf(fid, '    printf("\\n");\n');
    fprintf(fid, '}\n');
    fprintf(fid, '\n');
