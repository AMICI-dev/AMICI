...
function generateMainC(this)
    ...
    fprintf(fid, 'ReturnData getSimulationResults(UserData udata, ExpData edata, int *pstatus);\n');
    ...
    fprintf(fid, '\n');
    fprintf(fid, '    void *ami_mem = 0; /* pointer to cvodes memory block */\n');
    fprintf(fid, '    if (nx>0) {\n');
    fprintf(fid, '        ami_mem = setupAMI(pstatus, udata, tdata);\n');
    ...
    fprintf(fid, '\n');
    fprintf(fid, '    int problem = workForwardProblem(udata, tdata, rdata, edata, pstatus, ami_mem, &iroot);\n');
    fprintf(fid, '    if(problem)\n');
    fprintf(fid, '        goto freturn;\n');
    fprintf(fid, '\n');
    fprintf(fid, '    booleantype setupBdone = false;\n');
    fprintf(fid, '    problem = workBackwardProblem(udata, tdata, rdata, edata, pstatus, ami_mem, &iroot, &setupBdone);\n');
    fprintf(fid, '    if(problem)\n');
    fprintf(fid, '        goto freturn;\n');
    fprintf(fid, '\n');
    fprintf(fid, 'freturn:\n');
    fprintf(fid, '    storeJacobianAndDerivativeInReturnData(udata, tdata, rdata);\n');
    fprintf(fid, '    freeTempDataAmiMem(udata, tdata, ami_mem, setupBdone, *pstatus);\n');
    fprintf(fid, '\n');
