...
function generateCMakeFile(this)
    ...
    fprintf(fid, 'set(CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -Wall")\n');
    fprintf(fid, 'add_definitions(-DAMICI_WITHOUT_MATLAB)\n');
    
    % sources
    fprintf(fid, 'set(SRC_LIST\n');
    for f = {'main.c', 'wrapfunctions.c', '_symbolic_functions.c', '_amici.c', 'amidata.c'}
        fprintf(fid, '%s\n', f{1});
    end
    ...
    includeDirs = {fullfile(this.wrap_path, this.modelname), ...
        ...
        fullfile(ssparse_path, 'AND', 'Include'), ...
        fullfile(ssparse_path, 'SuiteSparse_config', 'Include'), ...
...
function generateMainC(this)
    ...
    fprintf(fid, '}\n');
    fprintf(fid, '\n');
    fprintf(fid, '// from amici.c setupUserData\n');
    fprintf(fid, '// call sol = ami_test1(tout,theta(1:2),kappa(1:0),options_ami, (4) plist, (5)pbar,xscale,data);\n');
    fprintf(fid, 'UserData getUserData() { // was: udata = setupUserData(prhs);\n');
    fprintf(fid, '    /**\n');
    fprintf(fid, '     * @brief setupUserData extracts information from the matlab call and returns the corresponding UserData struct\n');
    fprintf(fid, '     * @param[in] prhs: pointer to the array of input arguments @type mxArray\n');
    fprintf(fid, '     * @return udata: struct containing all provided user data @type UserData\n');
    fprintf(fid, '     */\n');
    fprintf(fid, '\n');
    fprintf(fid, '    // TODO run sanity check afterwards?\n');
    fprintf(fid, '\n');
    ...
    fprintf(fid, '    const int numTimepoints = 11; // TODO\n');
    fprintf(fid, '    ts = malloc(sizeof(realtype) * numTimepoints);\n');
    fprintf(fid, '    for(int i = 0; i < numTimepoints; ++i)\n');
    fprintf(fid, '        ts[i] = i;\n');
    fprintf(fid, '    nt = numTimepoints;\n');
    fprintf(fid, '\n');
    fprintf(fid, '    /* parameters, matlab: second argument */\n');
    fprintf(fid, '    const int sizeTheta = 2;\n');
    fprintf(fid, '    p = malloc(sizeof(realtype) * sizeTheta); // theta\n');
    fprintf(fid, '    p[0] = 1.5;\n');
    fprintf(fid, '    p[1] = 1;\n');
    fprintf(fid, '\n');
    fprintf(fid, '    /* constants, matlab: third argument */\n');
    fprintf(fid, '    k = malloc(sizeof(realtype)); // empty matrix if no fix parameters?!\n');
    fprintf(fid, '\n');
    fprintf(fid, '    /* Options ; matlab: fourth argument   */\n');
    fprintf(fid, '    // TODO OPTION treatment from simulate_$$.m\n');
    fprintf(fid, '    // TODO createAMIOption struct?\n');
    ...
    fprintf(fid, '\n');
    fprintf(fid, '    idlist = malloc(sizeof(realtype) * sizeTheta);\n');
    fprintf(fid, '    for(int i = 0; i < sizeTheta; ++i)\n');
    fprintf(fid, '        idlist[i] = 0;\n');
    fprintf(fid, '    z2event = 0; // []\n');
    fprintf(fid, '    qpositivex = malloc(sizeof(realtype) * sizeTheta);\n');
    fprintf(fid, '    for(int i = 0; i < sizeTheta; ++i)\n');
    fprintf(fid, '        qpositivex[i] = 0;\n');
    fprintf(fid, '    sensi = 0;\n');
    fprintf(fid, '    ism = 1;\n');
    ...
    fprintf(fid, '//    np = (int) mxGetM(prhs[4]) * mxGetN(prhs[4]);\n');
    fprintf(fid, '    np = sizeTheta;\n');
    fprintf(fid, '    // parameter reordering, matlab argument 5\n');
    fprintf(fid, '    // TODO check\n');
    fprintf(fid, '    plist = malloc(np*sizeof(int));\n');
    fprintf(fid, '    for (int ip=0; ip<np; ip++) {\n');
    fprintf(fid, '        plist[ip] = ip;\n');
    fprintf(fid, '    }\n');
    fprintf(fid, '\n');
    fprintf(fid, '    // TODO\n');
    fprintf(fid, '    //    if(mxGetProperty(prhs[3], 0 ,"sx0")) { sx0data = mxGetPr(mxGetProperty(prhs[3], 0 ,"sx0"));} else { }\n');
    fprintf(fid, '    sx0data = 0; //user-provided sensitivity initialisation. this should be a matrix of dimension [#states x #parameters] default is sensitivity initialisation based on the derivative of the state initialisation\n');
    fprintf(fid, '    //    if ((mxGetM(mxGetProperty(prhs[3], 0 ,"sx0")) * mxGetN(mxGetProperty(prhs[3], 0 ,"sx0")))>0) {\n');
    fprintf(fid, '    //        /* check dimensions */\n');
    fprintf(fid, '    //        if(mxGetN(mxGetProperty(prhs[3], 0 ,"sx0")) != np) { errMsgIdAndTxt("AMICI:mex:sx0","Number of rows in sx0 field does not agree with number of model parameters!"); }\n');
    fprintf(fid, '    //        if(mxGetM(mxGetProperty(prhs[3], 0 ,"sx0")) != nx) { errMsgIdAndTxt("AMICI:mex:sx0","Number of columns in sx0 field does not agree with number of model states!"); }\n');
    fprintf(fid, '    //        b_sx0 = TRUE;\n');
    fprintf(fid, '    //    } else {\n');
    ...
    fprintf(fid, '\n');
    fprintf(fid, '\n');
    fprintf(fid, '    //    /* pbar ; matlab: sixth argument*/\n');
    fprintf(fid, '    //    if (!prhs[5]) {\n');
    fprintf(fid, '    //        errMsgIdAndTxt("AMICI:mex:pbar","No parameter scales provided!");\n');
    fprintf(fid, '    //    }\n');
    fprintf(fid, '    //    pbar = mxGetPr(prhs[5]);\n');
    fprintf(fid, '    pbar = malloc(sizeof(realtype) * sizeTheta);\n');
    fprintf(fid, '    for(int i = 0; i < sizeTheta; ++i)\n');
    fprintf(fid, '        pbar[i] = 1;\n');
    fprintf(fid, '\n');
    fprintf(fid, '    //    /* xscale, matlab: seventh argument */\n');
    fprintf(fid, '    //    if (!prhs[6]) {\n');
    fprintf(fid, '    //        errMsgIdAndTxt("AMICI:mex:xscale","No state scales provided!");\n');
    fprintf(fid, '    //    }\n');
    fprintf(fid, '\n');
    ...
    fprintf(fid, '    }\n');
    fprintf(fid, '\n');
    fprintf(fid, '    int sizeTheta = 2;\n');
    fprintf(fid, '    int numTimepoints = 11;\n');
    fprintf(fid, '\n');
    fprintf(fid, '    // observation sizeTheta * numTimepoints\n');
    fprintf(fid, '    my = malloc(sizeof(realtype) * sizeTheta * numTimepoints);\n');
    fprintf(fid, '\n');
    fprintf(fid, '    // stdev of Y\n');
    fprintf(fid, '    ysigma = malloc(sizeof(realtype) * sizeTheta * numTimepoints);\n');
    fprintf(fid, '\n');
    fprintf(fid, '    // event observations\n');
    fprintf(fid, '    mz = malloc(sizeof(realtype));\n');
    fprintf(fid, '    zsigma = malloc(sizeof(realtype)); // stdev\n');
    fprintf(fid, '\n');
    ...
    fprintf(fid, '\n');
    fprintf(fid, 'void printReturnData(ReturnData rdata, UserData udata) {\n');
    fprintf(fid, '    int numTheta = udata->am_np;\n');
    fprintf(fid, '    int numTimepoints = udata->am_nt;\n');
    fprintf(fid, '\n');
    fprintf(fid, '    printf("tsdata\n");\n');
    fprintf(fid, '    for(int i = 0; i < numTimepoints; ++i)\n');
    fprintf(fid, '        printf("%f\t", rdata->am_tsdata[i]);\n');
    fprintf(fid, '\n');
    fprintf(fid, '    printf("\nxdotdata\n");\n');
    fprintf(fid, '    for(int i = 0; i < numTheta; ++i)\n');
    fprintf(fid, '        printf("%e\t", rdata->am_xdotdata[i]);\n');
    fprintf(fid, '\n');
    fprintf(fid, '\n');
    fprintf(fid, '    printf("\nxdata\n");\n');
    fprintf(fid, '    for(int i = 0; i < numTheta; ++i) {\n');
    fprintf(fid, '        for(int j = 0; j < numTimepoints; ++j)\n');
    fprintf(fid, '            printf("%f\t", rdata->am_xdata[j +  numTimepoints * i]);\n');
    fprintf(fid, '        printf("\n");\n');
    fprintf(fid, '    }\n');
    fprintf(fid, '\n');
    fprintf(fid, '    printf("\njdata\n");\n');
    fprintf(fid, '    for(int i = 0; i < numTheta; ++i) {\n');
    fprintf(fid, '        for(int j = 0; j < numTheta; ++j)\n');
    fprintf(fid, '            printf("%f\t", rdata->am_Jdata[i +  numTheta * j]);\n');
    fprintf(fid, '        printf("\n");\n');
    fprintf(fid, '    }\n');
    fprintf(fid, '\n');
    fprintf(fid, '\n');
    ...
    fprintf(fid, '    for(int i = 0; i < numTheta; ++i) {\n');
    fprintf(fid, '        for(int j = 0; j < numTimepoints; ++j)\n');
    fprintf(fid, '            printf("%f\t", rdata->am_ydata[j +  numTimepoints * i]);\n');
    fprintf(fid, '        printf("\n");\n');
    fprintf(fid, '    }\n');
    fprintf(fid, '\n');
    fprintf(fid, '    printf("\n");\n');
