function generateM(this, amimodelo2)
    ...
    switch(this.param)
        ...
        case 'log10'
            ...
            fprintf(fid,['    sol.srz = bsxfun(@times,sol.sz,permute(theta(options_ami.sens_ind),[3,2,1])*log(10));\n']);
            fprintf(fid,['    sol.ssigmay = bsxfun(@times,sol.ssigmay,permute(theta(options_ami.sens_ind),[3,2,1])*log(10));\n']);
            fprintf(fid,['    sol.ssigmaz = bsxfun(@times,sol.ssigmaz,permute(theta(options_ami.sens_ind),[3,2,1])*log(10));\n']);
        otherwise
    end
    fprintf(fid,'end\n');
    if(o2flag)
