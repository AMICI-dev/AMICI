...
def simulate_petab(
    ...
    if parameter_mapping is None:
        parameter_mapping = get_parameter_mapping(
            petab_problem, simulation_conditions, amici_model)
    ...
    # Fill in parameters in edatas (in-place)
    fill_in_parameters(
        edatas, problem_parameters, parameter_mapping, simulation_conditions,
...
def get_parameter_mapping(problem, simulation_conditions, amici_model):
    ...
    for (_, condition), prelim_mapping_for_condition in \
        ...
        mapping_for_condition = get_parameter_mapping_for_condition(
            prelim_mapping_for_condition, condition, problem)
...
def get_parameter_mapping_for_condition(
    ...
    species_in_condition_table = [
        col for col in problem.condition_df \
        if problem.sbml_model.getSpecies(col) is not None]
    ...
    if species_in_condition_table:
        ...
        def _set_initial_concentration(condition_id, species_id, init_par_id,
            ...
            value = petab.to_float_if_float(
                problem.condition_df.loc[condition_id, species_id])
            ...
            if isinstance(value, float):
                ...
                scale_map[init_par_id] = problem.parameter_df.loc[
    ...
    return (condition_map_preeq, condition_map_sim,
...
def get_edata_for_condition(
    ...
    measurement_df = petab.get_rows_for_condition(
        measurement_df=problem.measurement_df, condition=condition)
    ...
    if condition.get(PREEQUILIBRATION_CONDITION_ID) \
        ...
        edata.reiniitalizeFixedParameterInitialStates = True
...
def fill_in_parameters(
    ...
    for edata, mapping_for_condition, (_, condition) in \
        ...
        fill_in_parameters_for_condition(
            edata, problem_parameters, mapping_for_condition, condition,
            amici_model, scaled_parameters)


def fill_in_parameters_for_condition(
        edata, problem_parameters, parameter_mapping, condition, amici_model,
        scaled_parameters):
    (condition_map_preeq, condition_map_sim, condition_scale_map_preeq,
