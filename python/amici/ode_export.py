...
class ODEModel:
    ...
    def _generate_symbol(self, name: str) -> None:
        ...
        if name in self._variable_prototype:
            ...
            if name == 'w' and self.ncl() > 0:
                # w is created from the expression prototypes, but must be
                # correected for the number of conservation laws
                exps = (getattr(self, component))[0:-self.ncl()]
                self._syms[name] = sp.Matrix([comp.get_id() for comp in exps])
            else:
    ...
    def _equation_from_component(self, name: str, component: str) -> None:
        ...
        """
        if name == 'w' and self.ncl():
            self._eqs[name] = sp.Matrix(
                [comp.get_val() for comp in
                 getattr(self, component)[0:-self.ncl()]]
            )
        else:
            self._eqs[name] = sp.Matrix(
