...
def test_SplineUniform():
    spline = amici.splines.CubicHermiteSpline(
        ...
        xx=amici.splines.UniformGrid(0, 1, length=4),
        yy=[0.0, 2.0, 0.5, 1.0],
    )
...
def test_SplineExplicit():
    spline = amici.splines.CubicHermiteSpline(
        ...
        yy=[0.0, 2.0, 0.5,  1.0, 0.75],
        dd=[1.0, 0.0, 0.1, -0.1, 0.0],
    )
    assert math.isclose(float(spline.evaluate(0.00)), 0.0)
...
def test_SplineUniformSensitivity():
    ...
    spline = amici.splines.CubicHermiteSpline(
        ...
        x=amici.sbml_utils.amici_time_symbol,
        xx=amici.splines.UniformGrid(0, 1, length=4),
        yy=[3*a + b, c**2 - 3, 1, sp.log(b) + 3*c - 6*a],
    )
    check_gradient(spline, 0.00, params, params_values, [3.0, 1.0, 0.0])
    check_gradient(spline, 0.25, params, params_values, [0.539062, 0.179688, 4.45312], rel_tol=1e-5)
    ...
    check_gradient(spline, 0.75, params, params_values, [-1.07812, 0.179688, 0.1875], rel_tol=1e-5)
...
def test_SplineNonUniformSensitivity():
    ...
    check_gradient(spline, 0.00, params, params_values, [3.0,     1.0,     0.0])
    check_gradient(spline, 0.05, params, params_values, [1.3125,  0.4375,  2.89062], rel_tol=1e-5)
...
def test_SplineExplicitSensitivity():
    ...
    spline = amici.splines.CubicHermiteSpline(
        ...
        xx=amici.splines.UniformGrid(0, 1, length=4),
        yy=[3*a + b, c**2 - 3, 1, sp.log(b) + 3*c - 6*a],
        dd=[c**3 - 2, sp.sqrt(b) * sp.log(b) + 3*c, 4*a - sp.sin(b), 1],
    )
    check_gradient(spline, 0.00, params, params_values, [3.0,     1.0,     0.0])
    check_gradient(spline, 0.25, params, params_values, [0.46875,  0.109375,  4.37109], rel_tol=1e-6)
    check_gradient(spline, 1.0/3, params, params_values, [0.0,     0.0,     5.0])
    check_gradient(spline, 0.50, params, params_values, [-0.166667, 0.0641793, 2.625], rel_tol=1e-5)
    check_gradient(spline, 2.0/3, params, params_values, [ 0.0,     0.0,     0.0])
    check_gradient(spline, 0.75, params, params_values, [-0.75,     0.130923,  0.46875], rel_tol=1e-5)
    check_gradient(spline, 1.00, params, params_values, [-6.0,     1.0,     3.0])
...
def test_SplineLogarithmicSensitivity():
    ...
    check_gradient(spline, 0.00, params, params_values, [3.0,     1.0,     0.0])
    check_gradient(spline, 0.25, params, params_values, [0.585881,  0.195294,  4.38532], rel_tol=1e-5)
    check_gradient(spline, 1.0/3, params, params_values, [0.0,     0.0,     5.0])
    check_gradient(spline, 0.50, params, params_values, [0.514003, -0.132395,  1.52044], rel_tol=1e-5)
    check_gradient(spline, 2.0/3, params, params_values, [ 0.0,     0.0,     0.0])
    check_gradient(spline, 0.75, params, params_values, [-0.820743,  0.13679,  -0.0577988], rel_tol=1e-5)
    check_gradient(spline, 1.00, params, params_values, [-6.0,     1.0,     3.0])
