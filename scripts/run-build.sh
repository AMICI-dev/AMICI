...
cmake -DCMAKE_CXX_STANDARD=11 -DCMAKE_CXX_STANDARD_REQUIRED=ON -DCMAKE_ACTIVATE_COVERAGE=TRUE -DCMAKE_BUILD_TYPE=Debug ..
make
if [ $? -ne 0 ] ; then
exit 1
fi
...
mkdir -p build
cd build
cmake -DCMAKE_CXX_STANDARD=11 -DCMAKE_CXX_STANDARD_REQUIRED=ON -DCMAKE_BUILD_TYPE=Debug -DCMAKE_ACTIVATE_COVERAGE=TRUE -DCPPUTEST_DIR=${CPPUTEST_BUILD_DIR} ..
make
if [ $? -ne 0 ] ; then
exit 1
fi
