...
_model_jakstat_adjoint_py = amici._module_from_path(
    'model_jakstat_adjoint_py._model_jakstat_adjoint_py' if __package__ or '.' in __name__
    else '_model_jakstat_adjoint_py',
    Path(__file__).parent / f'_model_jakstat_adjoint_py{ext_suffix}',
)

def _get_import_time():
    return _model_jakstat_adjoint_py._get_import_time()
...
if t_imported < t_modified:
    ...
    raise RuntimeError(
        f'Cannot import extension for model_jakstat_adjoint_py from '
...
%module(package="model_jakstat_adjoint_py",moduleimport=MODULEIMPORT) model_jakstat_adjoint_py
