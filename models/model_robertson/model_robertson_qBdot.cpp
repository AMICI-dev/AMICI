...
int qBdot_model_robertson(realtype t, N_Vector x, N_Vector dx, N_Vector xB, N_Vector dxB, N_Vector qBdot, void *user_data) {
int status = 0;
...
status = dwdp_model_robertson(t,x,dx,user_data);
for(ip = 0; ip<udata->nplist; ip++) {
switch (udata->plist[ip]) {
  case 0: {
  qBdot_tmp[ip + udata->nplist*0] = x_tmp[0]*xB_tmp[0]-x_tmp[0]*xB_tmp[1];

  } break;

  case 1: {
  qBdot_tmp[ip + udata->nplist*0] = -xB_tmp[0]*tdata->dwdp[0]+xB_tmp[1]*tdata->dwdp[0];

  } break;

  case 2: {
  qBdot_tmp[ip + udata->nplist*0] = (x_tmp[1]*x_tmp[1])*xB_tmp[1];
