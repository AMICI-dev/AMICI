...
_model_events_py = amici._module_from_path(
    'model_events_py._model_events_py' if __package__ or '.' in __name__
    else '_model_events_py',
    Path(__file__).parent / f'_model_events_py{ext_suffix}',
)

def _get_import_time():
    return _model_events_py._get_import_time()
...
if t_imported < t_modified:
    ...
    raise RuntimeError(
        f'Cannot import extension for model_events_py from '
...
%module(package="model_events_py",moduleimport=MODULEIMPORT) model_events_py
