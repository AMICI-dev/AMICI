"""AMICI-generated module for model model_calvetti_py"""
...
if "1.1.0" != amici.__version__:
    raise amici.AmiciVersionError(
        f"Cannot use model `model_calvetti_py` in {Path(__file__).parent}, "
...
with warnings.catch_warnings():
    ...
    model_calvetti_py = amici._module_from_path(
        "model_calvetti_py.model_calvetti_py",
        Path(__file__).parent / "model_calvetti_py.py",
    )

for var in dir(model_calvetti_py):
    if not var.startswith("_"):
        globals()[var] = getattr(model_calvetti_py, var)

try:
    # _self: this module; will be set during import
    #  via amici.import_model_module
    model_calvetti_py._model_module = _self  # noqa: F821
except NameError:
    # when the model package is imported via `import`
    model_calvetti_py._model_module = sys.modules[__name__]
