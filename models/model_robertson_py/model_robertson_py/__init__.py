"""AMICI-generated module for model model_robertson_py"""
...
if "1.1.0" != amici.__version__:
    raise amici.AmiciVersionError(
        f"Cannot use model `model_robertson_py` in {Path(__file__).parent}, "
...
with warnings.catch_warnings():
    ...
    model_robertson_py = amici._module_from_path(
        "model_robertson_py.model_robertson_py",
        Path(__file__).parent / "model_robertson_py.py",
    )

for var in dir(model_robertson_py):
    if not var.startswith("_"):
        globals()[var] = getattr(model_robertson_py, var)

try:
    # _self: this module; will be set during import
    #  via amici.import_model_module
    model_robertson_py._model_module = _self  # noqa: F821
except NameError:
    # when the model package is imported via `import`
    model_robertson_py._model_module = sys.modules[__name__]
