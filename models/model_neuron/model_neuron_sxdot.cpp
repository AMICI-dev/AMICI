...
if(ip == 0) {
    status = JSparse_model_neuron(t,x,xdot,udata->J,user_data,NULL,NULL,NULL);
    status = dxdotdp_model_neuron(t,x,NULL,user_data);
}
  sxdot_tmp[0] = udata->dxdotdp[0 + ip*udata->nx]+udata->J->data[0]*sx_tmp[0]+udata->J->data[2]*sx_tmp[1];
  sxdot_tmp[1] = udata->dxdotdp[1 + ip*udata->nx]+udata->J->data[1]*sx_tmp[0]+udata->J->data[3]*sx_tmp[1];
return(status);
