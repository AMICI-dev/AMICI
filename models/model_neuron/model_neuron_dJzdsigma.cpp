...
if(!amiIsNaN(edata->mz[0*udata->nmaxevent+tdata->nroots[ie]])){
    iz = 0;
  tdata->dJzdsigma[iz+(0)*udata->nztrue] = 1.0/(tdata->sigmaz[0]*tdata->sigmaz[0]*tdata->sigmaz[0])*pow(edata->mz[tdata->nroots[ie]+udata->nmaxevent*0]-rdata->z[tdata->nroots[ie]+udata->nmaxevent*0]*1.0,2.0)*-1.0+1.0/tdata->sigmaz[0];
}
return(status);
