...
TEST_F(ExpDataTest, MeasurementCtor) {
    ...
    checkEqualArray(
        z_std, edata.get_event_measurement_error(), TEST_ATOL, TEST_RTOL,
    ...
    checkEqualArray(
        edata_copy.get_measurement_error(),
        edata.get_measurement_error(), TEST_ATOL, TEST_RTOL,
        "observedDataStdDev"
    );
    checkEqualArray(
    ...
    checkEqualArray(
        edata_copy.get_event_measurement_error(),
        edata.get_event_measurement_error(), TEST_ATOL, TEST_RTOL,
        "observedEventsStdDev"
    );
}
...
TEST_F(ExpDataTest, DimensionChecks) {
    ...
    ASSERT_THROW(edata.set_measurement_error(bad_y_std), AmiException);
    ASSERT_THROW(edata.set_event_measurements(bad_z), AmiException);
    ASSERT_THROW(edata.set_event_measurement_error(bad_y_std), AmiException);
    ...
    ASSERT_THROW(
        edata.set_measurement_error(bad_single_y_std, 0), AmiException
    );
    ASSERT_THROW(edata.set_event_measurements(bad_single_z, 0), AmiException);
    ASSERT_THROW(
        edata.set_event_measurement_error(bad_single_y_std, 0), AmiException
    );
...
TEST_F(ExpDataTest, SettersGetters) {
    ...
    edata.set_measurement_error(y_std);
    checkEqualArray(
        edata.get_measurement_error(), y_std, TEST_ATOL, TEST_RTOL,
        "ObservedDataStdDev"
    );
    ...
    edata.set_event_measurement_error(z_std);
    checkEqualArray(
        edata.get_event_measurement_error(), z_std, TEST_ATOL, TEST_RTOL,
        "ObservedEventsStdDev"
    );
    ...
    for (int iy = 0; iy < ny; ++iy) {
        ...
        edata.set_measurement_error(single_y_std, iy);
    }
    ...
    ASSERT_THROW(
        edata.set_measurement_error(single_y_std, ny), std::exception
    );
    ASSERT_THROW(
        edata.set_measurement_error(single_y_std, -1), std::exception
    );
    ...
    for (int iz = 0; iz < nz; ++iz) {
        ...
        edata.set_event_measurement_error(single_z_std, iz);
    }
    ...
    ASSERT_THROW(
        edata.set_event_measurement_error(single_z_std, nz), std::exception
    );
    ASSERT_THROW(
        edata.set_event_measurement_error(single_z_std, -1), std::exception
    );

    ASSERT_TRUE(edata.get_measurements_ptr(0));
    ASSERT_TRUE(edata.get_measurement_error_ptr(0));
    ASSERT_TRUE(edata.get_event_measurements_ptr(0));
    ASSERT_TRUE(edata.get_event_measurement_error_ptr(0));

    std::vector<realtype> empty(0, 0.0);

    edata.set_measurements(empty);
    edata.set_measurement_error(empty);
    edata.set_event_measurements(empty);
    edata.set_event_measurement_error(empty);
    ...
    checkEqualArray(
        edata.get_measurement_error(), empty, TEST_ATOL, TEST_RTOL,
        "ObservedDataStdDev"
    );
    checkEqualArray(
    ...
    checkEqualArray(
        edata.get_event_measurement_error(), empty, TEST_ATOL, TEST_RTOL,
        "ObservedEventsStdDev"
    );
}
