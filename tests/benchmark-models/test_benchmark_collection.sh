...
for model in $models; do
    ...
    printf '=%.0s' {1..40}
    echo
    yaml="${BENCHMARK_COLLECTION}"/"${model}"/"${model}".yaml
    amici_model_dir=test_bmc/"${model}"
    mkdir -p "$amici_model_dir"
    amici_import_petab --verbose -y "${yaml}" -o "${amici_model_dir}" -n "${model}"
    "$script_path"/test_petab_model.py --verbose -y "${yaml}" -d "${amici_model_dir}" -m "${model}" -c
    printf '=%.0s' {1..100}
    echo
    echo
done
