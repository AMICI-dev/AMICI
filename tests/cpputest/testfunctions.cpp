...
#endif
        DOUBLES_EQUAL(expected[i], actual[i], epsilon);
    }
}
...
//    delete[] expected;
    ...
    checkEqualArray(expected, rdata->am_ydata, udata->am_nt, epsilon);
    delete[] expected;

    AMI_HDF5_getDoubleArrayAttribute2D(file_id, resultPath, "sigmay", &expected, &m, &n);
    checkEqualArray(expected, rdata->am_sigmaydata, udata->am_nt, epsilon);
    delete[] expected;

    AMI_HDF5_getDoubleArrayAttribute2D(file_id, resultPath, "xdot", &expected, &m, &n);
    checkEqualArray(expected, rdata->am_xdotdata, udata->am_nx, epsilon);
    delete[] expected;
...
//        delete[] expected;
        ...
        if(udata->am_sensi_meth == AMI_SENSI_FSA) {
            ...
            checkEqualArray(expected, rdata->am_sxdata, udata->am_nt * udata->am_nx * udata->am_nplist, epsilon);
            delete[] expected;

            AMI_HDF5_getDoubleArrayAttribute3D(file_id, resultPath, "sy", &expected, &m, &n, &o);
            checkEqualArray(expected, rdata->am_sydata, udata->am_nt * udata->am_ny * udata->am_nplist, epsilon);
            delete[] expected;
