function this = gccode(this,model,fid)
    ...
    if(any(any(any(this.sym~=0))))
        ...
        if(numel(cstr)>1)
            ...
            cstr = regexprep(cstr,'var_stau_([0-9]+)','stau[ip]');
            ...
            cstr = regexprep(cstr,'var_dydp_([0-9]+)',['dydp[ip*' num2str(model.ny) ' + $1]']);    
            cstr = regexprep(cstr,'var_dzdp_([0-9]+)',['dzdp[ip*' num2str(model.nz) ' + $1]']);
            cstr = regexprep(cstr,'var_drzdp_([0-9]+)',['drzdp[ip*' num2str(model.nz) ' + $1]']);
            cstr = regexprep(cstr,'var_drootdp_([0-9]+)',['drzdp[ip*' num2str(model.nz) ' + $1]']);
