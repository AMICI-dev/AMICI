...
class JAXModel_test_noise_params_jax(JAXModel):
    api_version = '0.0.4'
    ...
    def _sigmay(self, y, p, np):
        ...
        sigma_obsA = noiseParameter1
        sigma_obsB = noiseParameter1

        return jnp.array([sigma_obsA, sigma_obsB])
    ...
    def _nllh(self, t, x, p, tcl, h, my, iy, op, np):
        ...
        obsA, obsB,  = y
        sigma_obsA, sigma_obsB,  = self._sigmay(y, p, np)

        _amici_cse_0 = sigma_obsA**2
        _amici_cse_1 = 2*jnp.pi
        _amici_cse_2 = sigma_obsB**2
        Jy0 = 0.5*safe_log(_amici_cse_0*_amici_cse_1) + safe_div(0.5*(-my + obsA)**2, _amici_cse_0)
        Jy1 = 0.5*safe_log(_amici_cse_1*_amici_cse_2) + safe_div(0.5*(-my + obsB)**2, _amici_cse_2)
...
Model = JAXModel_test_noise_params_jax
